...
Page({
  ...
  getStatDateRange() {
    ...
    var today = new Date()
    var range = {begin:new Date(), end:new Date()}
  ...
  queryTables(range, callback) {
    const self = this
    const s = self.data.career.statistics
    s.result = { count: { total: 0, leader: 0, loaded: 0 }, level: { total: 0, average:0, max: { value: 0, id:"", title:"" } }, added: { up: 0, length: 0 }, star:0 }
    // 先看 我参与的活动和 我发起的活动，
    var outdoors = []
    var num = 0
    dbPersons.doc(app.globalData.personid).get().then(res => {
      ...
      outdoors.forEach((item, index) => {
        ...
        .then(res => {
          ...
          if ((theDate >= range.begin && theDate <= range.end) // 去掉时间范围不对的 
            ...
            var isTest = false
            if (res.data.limits && res.data.limits.isTest == true) {
              isTest = true
            }
            if(!isTest) { // 去掉活动为测试类的
